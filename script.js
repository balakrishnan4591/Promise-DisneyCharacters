async function fetchDisneyCharacter() {
  try {
    //fetch the api
    let response = await fetch("https://api.disneyapi.dev/character");

    let disneyCharacters = await response.json();
    ...
    let count = disneyCharacters.data.length;
    ...
    for (i = 0; i < count; i++) {
      ...
      // cardContent.setAttribute("class", "card-text-content");
      ...
      cardImg.src = disneyCharacters.data[i].imageUrl;
      cardContentHeading.innerHTML = disneyCharacters.data[i].name;
      cardBodyHearder.innerHTML = "Disnep Charater " + (i + 1);
      // cardContentCapital.innerHTML = ": ";
      cardContentRegion.innerHTML = "Films : " + disneyCharacters.data[i].films;
